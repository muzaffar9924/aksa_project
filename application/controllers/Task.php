...
class Task extends CI_Controller
{
	...
	public function add_task()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$query = $this->db->query("INSERT INTO `employee_task`(`assign_by`,`assign_to`,`emp_name`, `company_name`, `task_name`, `allocation_date`, `start_date`, `start_time`,`end_date`, `end_time`, `work_status`) VALUES ('$assign_by','$assign_to','$emp_name','$company','$task_name','$allocation_date','$date_start','$start_time','$date_ends','$end_time','$work_status')");
	...
	function assign_task()
	{
		...
		if ($this->Auth->authorizedUser() == false) {
			...
			$select_query = $this->db->query("SELECT * FROM `employee_task` where  `assign_by` = '$emp_name' ORDER BY `user_id` DESC");
			...
			// echo "<pre>";
			// die();
			$select_pending = $this->db->query("SELECT * FROM `employee_task` WHERE `assign_by` = '$emp_name' AND `work_status`='Pending'");

			$select_onprogress = $this->db->query("SELECT * FROM `employee_task` WHERE `assign_by` = '$emp_name' AND  `work_status`='In-progress'");

			$select_complete = $this->db->query("SELECT * FROM `employee_task` WHERE `assign_by` = '$emp_name' AND  `work_status`='Completed'");
	...
	function assignPendingTask()
	{
		...
		$data['result'] = $this->TaskModel->assignPendingTask($userName);
		...
		$this->load->view('assign_pending_task', $data);
	}
